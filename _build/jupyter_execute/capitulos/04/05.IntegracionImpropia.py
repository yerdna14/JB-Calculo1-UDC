...
# Estas integrales de denominan impropias y pueden clasificarse en:
...
# 
# En estas integrales, el intervalo de integración no es finito.  Sea $f: [a,+\infty) \rightarrow \mathbb{R}$ integrable en $[a, b]$ para cualquier $b \geq a$.  Entonces, se define:
# \begin{equation*}
# \int_a^{+\infty} f(x) \, dx = \underset{b \rightarrow +\infty}{\lim} \int_a^b f(x) \, dx \, .
# \end{equation*}
# Su cálculo tiene dos partes:
# - calculamos la integral definida en un intervalo finito
# - hacemos que el extremo superior del intervalo tienda a infinito.
# 
# Veamos un ejemplo:
# \begin{equation*}
...
# Si el intervalo no está acotado en ninguno de sus extremos, basta con tomar dos límites.  Por ejemplo,
# \begin{align*}
# \int_{-\infty}^{+\infty} \frac{dx}{x^2+1} &= \underset{a \rightarrow -\infty}{\lim} \underset{b \rightarrow +\infty}{\lim} \int_a^b \frac{dx}{x^2+1} = \\
#     &= \underset{a \rightarrow -\infty}{\lim} \underset{b \rightarrow +\infty}{\lim} \left[ \arctan (x) \right]_a^b = \underset{a \rightarrow -\infty}{\lim} \underset{b \rightarrow +\infty}{\lim} \left[ \arctan (b) - \arctan (a) \right] = \frac{\pi}{2} - \left( -\frac{\pi}{2} \right) = \pi \, .
# \end{align*}
# De manera equivalente, podemos descomponer la integral en dos integrales impropias donde -en cada una de ellas- solo uno de los extremos no está acotado:
# \begin{align*}
...
# \end{align*}
# De manera similar haríamos si la función tiende a infinito cuando nos acercamos al límite derecho del intervalo:
# \begin{equation*}
...
#                                  + \underset{a \rightarrow 2}{\lim} \int_b^4 (x-2)^{-1/3} \, dx = \\
#     &= \underset{b \rightarrow 2}{\lim} \left[ \frac{(x-2)^{2/3}}{2/3} \right]_1^b
#      + \underset{a \rightarrow 2}{\lim} \left[ \frac{(x-2)^{2/3}}{2/3} \right]_b^4 = \\
#     &= \frac{3}{2} \underset{b \rightarrow 2}{\lim} \left[ (b-2)^{2/3} - (1-2)^{2/3} \right]
#      + \frac{3}{2} \underset{a \rightarrow 2}{\lim} \left[ (4-2)^{2/3} - (1-2)^{2/3} \right] = \\
...
# 
# En este grupo incluimos las integrales en las que tanto el intervalo como la función dejan de estar acotados.  Para resolverlas, basta con combinar las técnicas que hemos visto en las secciones anteriores.
# 
# Veamos un ejemplo:
# \begin{equation*}
# \int_1^{+\infty} \frac{dx}{(x-4)^{2/5}} \, .
# \end{equation*}
# Vemos que el intervalo de integración es infinito y, además, la función a integrar no está acotada en $x = 4$.  Para resolverla, la descomponemos en:
...
#      + \underset{c \rightarrow +\infty}{\lim} \int_6^c (x-4)^{-2/5} \, dx = \\
...
#                          + \underset{a \rightarrow 4}{\lim} \left( (6-4)^{3/5} - (a-4)^{3/5} \right) 
#                          + \underset{c \rightarrow +\infty}{\lim} \left( (c-4)^{3/5} - 2^{3/5} \right) \right] = \\
#     &= \dfrac{5}{3} \left[ 0 + 3^{3/5} + 2^{3/5} - 0 
#                     + \underset{c \rightarrow +\infty}{\lim} (c-4)^{3/5} - 2^{3/5} \right] = +\infty \, .
# \end{align*}
# Por lo tanto, en este caso tenemos una integral divergente.
