...
# 
# **Definición:**
# Sea $f:(a,b) \longrightarrow \mathbb R$ una función derivable
# 
# Es decir, $f$ es continua en $x_0$ si y sólo si en $(a,b)$. Definimos la función derivada como:
#              \begin{equation*}
#              \begin{array}{cccc}
...
#     f'''(x_0) =(f'')'(x_0) = \lim_{h\rightarrow 0} \dfrac{f''(x_0+h) - f''(x_0)}{h}.
#  \end{equation*}
#  Así podríamos definir la derivada cuarta, etc. En general, una vez que se tiene $f^{n}:(a,b) \longrightarrow \mathbb R$, se define: 
#  \begin{equation*}
#  f^{n+1}(x_0) = \left( f^{(n)}\right)'(x_0).
#  \end{equation*}
#  
#  Los ejemplos son sencillos en este caso:
# * $f(x) = \sin(x)+x^2$,
# * $f'(x) = \cos(x) + 2x$,
# * $f''(x) = -\sin(x) + 2$,
# * $f'''(x) = - \cos(x)$,
...
# **Definición** *Clase de una función*. Sea $f:(a,b):\longrightarrow \mathbb R$. Diremos que
...
# 
# **Ejercicio:**
# Sea la función $f(x) = \dfrac12 x |x|$, $\forall x\in\mathbb R$. ¿Cuál es la clase de $f$ en $\mathbb R$?
# 
...
# \end{array}\right.
# $$
# La representación gráfica de $f$ se puede obtener del siguiento modo:
...
# &&\lim_{x\to 0^{+}}f(x)=\lim_{x\to 0^{+}}\frac{1}{2}x^2=0=f(0).
# \end{eqnarray*}
# Como los dos límites coinciden con el valor de la función en ese punto, ya sabemos que $f$ es continua en $0$ y, por tanto, continua en todo $\mathbb R$. Entonces $$\fbox{$f\in\mathcal{C}^0(\mathbb R).$}$$
...
# \end{array}\right.
# $$
# Es muy sencillo comprobar que $f'$ es una función continua. Entonces $$\fbox{$f\in\mathcal{C}^1(\mathbb R).$}$$
